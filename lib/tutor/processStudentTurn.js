...
export async function processStudentTurn({ problemState, studentText }) {
  ...
  if (
    priorHelpContext?.active &&
    priorHelpContext?.kind === "tutorial" &&
    routeResult?.route === "accept_step" &&
    routeResult?.match?.stepType === "move_x_term" &&
    typeof routeResult?.match?.valueRaw === "string" &&
    routeResult.match.valueRaw.trim().toLowerCase() !== "x"
  ) {
