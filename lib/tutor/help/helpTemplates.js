function inferEquationFamily(problemState) {
  ...
  const equationText =
    problemState?.currentEquation || problemState?.originalEquation || "";

  const parts = equationText.split("=");
  const rightSide = parts[1] || "";

  if (/x/i.test(rightSide)) {
    return "ax_plus_b_eq_cx_plus_d";
  }
...
export function getHelpTemplate(problemState) {
  ...
  return {
    ...
    prompt: "For your problem, type the next step in words to remove the constant term on the left side (for example: subtract 5)"
  };
}