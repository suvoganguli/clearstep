  ...
  if (route === "accept_step") {
    if (mathResult?.kind === "STEP_INCORRECT") {
      if (priorHelpContext?.active && priorHelpContext?.kind === "tutorial") {
        return {
          tutorText:
            "That is not the right step here. You may need to subtract the correct term or divide by the correct term.",
          status: "active",
        };
      }

      return {
        tutorText: "That is not the right step here. You may need to subtract the correct term or divide by the correct term.",
        status: "active",
      };
    }
