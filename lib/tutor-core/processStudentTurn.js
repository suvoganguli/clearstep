...
import { detectOutOfScopeFractionInput } from "../algebra/common/fractionInputGuard.js";
...
export async function processStudentTurn({ problemState, studentText }) {
  ...
  if (problemState.awaitingConfirmation && isYes(studentText)) {
    ...
    const fractionIssue = detectOutOfScopeFractionInput(studentText);
    if (fractionIssue) {
      ...
      const nextState = appendHistory(nextStateBase, {
        studentText,
      ...
      return {
        ...
        mathResult: null,
        reply: {
          tutorText: fractionIssue.message,
