export function buildTutorReply({ routeResult, mathResult, priorHelpContext }) {
  const { route, match } = routeResult;
    ...
    if (route === "help_question") {
      ...
      if (match?.stepType === "ask_for_help") {
        return {
          tutorText:
            "Good question. First choose what you want to undo: a constant term, an x-term, or a coefficient. Then type the step in words, for example: subtract 5.",
          status: "active",
        };
      }
  ...
  if (route === "ask_for_answer") {
    return {
      tutorText:
        "I won’t give the final answer directly, but I can help you get there step by step. Tell me what you want to undo first, or type \"help\".",
      status: "active",
    };
  }
  ...
  if (route === "accept_step") {
    ...
    if (match?.stepType === "state_final_answer") {
      ...
      if (mathResult?.kind === "FINAL_INCORRECT") {
        return {
          tutorText: "Not quite. Try again and tell me your answer in the form x = ...",
          status: "active",
        };
      }
