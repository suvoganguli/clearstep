export function deterministicStepMatcher(studentText) {
  ...
  if (!text) {
    ...
    let m = text.match(/\bsubtract\s+([+-]?\d*)x\b/i);
  if (m) {
    const coef = m[1];
    const valueRaw =
      coef === "" || coef === "+"
  ...
  if (
    ...
    /^\s*(subtract|divide|multiply)\s*\?\s*$/i.test(text) ||
    /^\s*subtract\s+(?:a\s+)?constant\s*\?\s*$/i.test(text)
  ) {
    return {
      source: "deterministic",
