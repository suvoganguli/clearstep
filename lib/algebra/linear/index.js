...
import {
  solveLinear,
  checkNextStep,
  deriveNextEquationFromMathResult,
} from "./v1/engine.js";

export { canonicalizeLinearEquation } from "./v1/canonicalize.js";

/**
 * Linear Algebra Router (Phase 1)
 * - v1 supports: ax + b = c (integer or decimal coefficients; fractions not yet)
...
export function deriveNextEquationFor(version, solved, mathResult) {
  if (version === "v1")
    return deriveNextEquationFromMathResult(solved, mathResult);
  throw new Error(`Unsupported linear version: ${version}`);
}
