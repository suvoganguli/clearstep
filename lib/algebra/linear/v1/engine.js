...
// Match parser/canonicalize: decimals only, not fractions like 2/3.
...
export function normalizeStepInput(studentText) {
  ...
  let m = t.match(
    new RegExp(String.raw`^(?:is\s+)?x\s*=\s*([+-]?(?:${DEC}))\s*\??$`, "i"),
  );
  if (m) return { kind: "X_EQUALS", x: parseFloat(m[1]) };

  m = t.match(
    new RegExp(
      String.raw`^(?:is\s+)?([+-]?(?:${DEC})?)x\s*=\s*([+-]?(?:${DEC}))\s*\??$`,
      "i",
    ),
  );
  if (m) {
  ...
  if (new RegExp(String.raw`^[+-]?(?:${DEC})$`).test(t)) {
    return { kind: "NUMBER", rhs: parseFloat(t) };
  }

  m = t.match(
    new RegExp(String.raw`\bdivide\s+by\s+([+-]?(?:${DEC}))\b`, "i"),
  );
  if (m) return { kind: "OP_DIVIDE", n: parseFloat(m[1]) };

  m = t.match(
    new RegExp(String.raw`\bsubtract\s+([+-]?(?:${DEC}))\b`, "i"),
  );
  if (m) return { kind: "OP_SUBTRACT", n: parseFloat(m[1]) };

  m = t.match(new RegExp(String.raw`\badd\s+([+-]?(?:${DEC}))\b`, "i"));
  if (m) return { kind: "OP_ADD", n: parseFloat(m[1]) };

  m = t.match(
    new RegExp(String.raw`\bsubtract\s+([+-]?(?:${DEC})?)x\b`, "i"),
  );
  if (m) return { kind: "OP_SUBTRACT_X", n: parseCoef(m[1]) };

  m = t.match(new RegExp(String.raw`\badd\s+([+-]?(?:${DEC})?)x\b`, "i"));
...
export function checkNextStep(studentMessage, solved) {
  ...
  if (parsed.kind === "OP_DIVIDE") {
    ...
    return {
      kind: "STEP_INCORRECT",
      expected: `divide by ${formatNumberForHint(solved.a)}`,
    };
  }
