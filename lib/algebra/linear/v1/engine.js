...
export function normalizeStepInput(studentText) {
  ...
  let m = t.match(/^(?:is\s+)?x\s*=\s*([+-]?\d+)\s*\??$/i);
  if (m) return { kind: "X_EQUALS", x: parseInt(m[1], 10) };

  m = t.match(/^(?:is\s+)?([+-]?\d*)x\s*=\s*([+-]?\d+)\s*\??$/i);
  if (m) {
    return {
      kind: "AX_EQUALS",
      a: parseCoef(m[1]),
...
export function checkNextStep(studentMessage, solved) {
  ...
  if (parsed.kind === "OP_SUBTRACT_X") {
    ...
    return {
      kind: "STEP_INCORRECT",
      expected: `subtract ${solved.rightXCoeff === 1 ? "x" : solved.rightXCoeff + "x"}`,
    };
  }

  if (parsed.kind === "OP_ADD_X") {
    if (solved.rightXCoeff < 0 && parsed.n === Math.abs(solved.rightXCoeff)) {
      return {
        kind: "STEP_HINT",
        stage: "MOVE_X_TERMS",
        n: solved.rightXCoeff,
