...
export function normalizeStepInput(studentText) {
  const t = (studentText || "").trim().toLowerCase();

  // x = 5
  const xeq = t.match(/^x\s*=\s*(-?\d+)$/);
  if (xeq) {
    return { kind: "X_EQUALS", x: parseInt(xeq[1], 10) };
  }

  // 3x = 15
  const eq = t.match(/^(-?\d+)x\s*=\s*(-?\d+)$/);
  if (eq) {
    return { kind: "EQUATION_FORM", a: parseInt(eq[1], 10), rhs: parseInt(eq[2], 10) };
  }

  // Just a number like "15" or "5"
  if (/^-?\d+$/.test(t)) {
...
export function parseAction(studentText) {
  const t = (studentText || "").trim().toLowerCase();

  // subtract 5, minus 5
  let m = t.match(/^(subtract|minus)\s+(-?\d+)$/);
  if (m) {
    return { kind: "ACTION", op: "ADD_BOTH_SIDES", value: -parseInt(m[2], 10) };
  }

  // add 5, plus 5
  m = t.match(/^(add|plus)\s+(-?\d+)$/);
  if (m) {
    return { kind: "ACTION", op: "ADD_BOTH_SIDES", value: parseInt(m[2], 10) };
  }

  // divide by 3, divide 3
  m = t.match(/^divide(\s+by)?\s+(-?\d+)$/);
  if (m) {
    return { kind: "ACTION", op: "DIVIDE_BOTH_SIDES", value: parseInt(m[2], 10) };
  }

  // multiply by 3
  m = t.match(/^multiply(\s+by)?\s+(-?\d+)$/);
  if (m) {
    return { kind: "ACTION", op: "MULTIPLY_BOTH_SIDES", value: parseInt(m[2], 10) };
  }
...
export function checkNextStep(studentText, solved) {
  ...
  const action = parseAction(studentText);

  // Phase 1A: expected actions are:
  // Step 1: add (-b) to both sides
  // Step 2: divide both sides by a
  if (action.kind === "ACTION") {
    // If they try to "add/subtract", validate it as Step 1
    if (action.op === "ADD_BOTH_SIDES") {
      const expected = -solved.b;

      if (action.value === expected) {
        return { ok: true, kind: "STEP1_ACTION_CORRECT", expectedNext: "DIVIDE_BY_A" };
      }

      return {
        ok: false,
        kind: "STEP1_ACTION_INCORRECT",
        message: `To remove ${solved.b}, you should add ${expected} to both sides.`,
      };
    }

    // If they try to divide, validate as Step 2
    if (action.op === "DIVIDE_BOTH_SIDES") {
      const expected = solved.a;

      if (action.value === expected) {
        return { ok: true, kind: "STEP2_ACTION_CORRECT", expectedNext: "FINAL" };
      }

      return {
        ok: false,
        kind: "STEP2_ACTION_INCORRECT",
        message: `To isolate x, you should divide both sides by ${expected}.`,
      };
    }

    // For now, block multiply (we may allow later)
    return {
      ok: false,
      kind: "ACTION_NOT_SUPPORTED",
      message: "For now, use add/subtract and divide steps to solve the equation.",
    };
  }
