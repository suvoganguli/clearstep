...
function extractEmbeddedStep(text) {
  ...
  // Detect a full linear equation entered as a step, e.g. "2x + 5 = 9"
  if (/^[^=]+=[^=]+$/.test(s)) {
    return s.replace(/\s+/g, "");
  }
  ...
  // Detect subtracting an x-term (e.g., "subtract x", "subtract 2x", "subtract -x")
  const subtractXMatch = s.match(/\bsubtract\s+([+-]?\d*)x\b/);
  if (subtractXMatch) {
    const coef = subtractXMatch[1] || "";
    return `subtract ${coef}x`;
  }
  ...
  // Detect adding an x-term (e.g., "add x", "add 2x", "add -x")
  const addXMatch = s.match(/\badd\s+([+-]?\d*)x\b/);
  if (addXMatch) {
    const coef = addXMatch[1] || "";
    return `add ${coef}x`;
  }
